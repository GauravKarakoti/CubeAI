...
from Backend.genral_recommend import main
import subprocess
import threading
import json
import logging
...
def option_recommender():
    ...
    ShowTextToScreen(f"{Assistantname}: Choose General Recomendation or Specialized Recomendation")
    TextToSpeech("Choose General Recomendation or Specialized Recomendation")
    SetAssistantStatus("Listening ... ")
    revert= safe_speech_recognition()
    ShowTextToScreen(f"{Username}: {revert}")
    if  "general" in revert.lower():
        main()
    elif "specialized" in revert.lower():
...
def MainExecution():
    ...
    if "suggest profession" in Query.lower() or "suggest profession" in Query.lower() or "career recomendation" in Query.lower():
        option_recommender()
    else:
